...
const rootBuild = () =>
  src([`${config.src.assets.root}/robots.txt`, `${config.src.assets.root}/manifest.json`], {
    allowEmpty: true,
...
// Переносит в build/libs (локальные библиотеки)
const libsBuild = () =>
  src([`${config.src.assets.libs}/**/*`], {
    encoding: false,
  }).pipe(dest(`${config.build.libs}`))

// Выполнение всех тасков
export const assetsBuild = series(rootBuild, fontsBuild, videosBuild, libsBuild)
