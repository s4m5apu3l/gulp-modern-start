...
const server = done => {
  browserSync.init({
    ...
    // ОПТИМИЗАЦИЯ: Отключаем ненужные файлы от отслеживания
    // BrowserSync не должен следить за файлами - это делает Gulp
    watchOptions: {
      ignoreInitial: true,
      ignored: ['*.txt', '*.json', 'node_modules/**'],
    },
    
    // ОПТИМИЗАЦИЯ: Настройки для CSS injection без reload
    injectChanges: true, // инжектить CSS без перезагрузки
    
    // ОПТИМИЗАЦИЯ: Уменьшаем задержку обновления
    reloadDelay: 0,
    reloadDebounce: 0,
  })
